...
SERIAL_PORT = '/dev/cu.usbserial-1410'  # Cambia esto al puerto de tu sensor (e.g., 'COM4')
BAUD_RATE = 115200
...
def calibrate_offset(ser, num_samples=200):
            ...
            if line:
                ...
                if len(parts) == 3:
                    try:
                        ...
                        print(f"Flujo crudo: {flujo:.4f} slm, Flujo filtrado: {flujo_filtered:.4f} slm, Burbujas: {burb_count}, Volumen Total: {volume_total:.4f} L, Volumen Promedio por Burbuja: {vol_por_burbuja_avg:.4f} L")

                        if new_bubbles > 0:
                            print(f"Nueva(s) burbuja(s) detectada(s): {new_bubbles}, Volumen individual aproximado: {vol_per_bubble:.4f} ")
                    except ValueError:
